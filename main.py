...
def build_model(img_width: int, img_height: int, img_channels: int, output_dim: int) -> Model:
    img_input = keras.Input(shape=(img_width, img_height, img_channels))
    print(img_input.shape)

    base_model = ResNet50(input_tensor=img_input, weights=None, include_top=False)
    x = base_model.output
...
def train_model(model: Model, train_data_generator: DataGenerator.CustomSequence, val_data_generator: DataGenerator.CustomSequence,
                batch_size: int, learn_rate: float, initial_epoch: int, epochs: int):
    lr_scale_factor = 0.5
    log_rate = 10
    experiment_rootdir = "checkpoints"
    ...
    save_model_and_loss = log_utils.MyCallback(filepath=experiment_rootdir, period=log_rate, batch_size=batch_size, factor=lr_scale_factor)
    ...
    print("Train {}, vali {}, batch {}".format(train_data_generator.samples, val_data_generator.samples, batch_size))
    print("Step per epoch {}".format(steps_per_epoch))
    print("Validation steps {}".format(validation_steps))

    model.fit(train_data_generator, batch_size=batch_size,
...
def _main(flags: argparse) -> None:
    ...
    train_image_loader = DataGenerator.CustomSequence(flags.train_dir, flags.frame_mode, True, (img_height, img_width), batch_size, False)
    val_image_loader = DataGenerator.CustomSequence(flags.train_dir, flags.frame_mode, False, (img_height, img_width), batch_size, False)

    model = build_model(img_height, img_width, img_channels, 1)
    train_model(model, train_image_loader, val_image_loader, batch_size, learn_rage, initial_epoch, epochs)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r", "--random_seed", help="Initial random seed", type=int, default=100)
    ...
    parser.add_argument("-e", "--epochs", help="Number of epochs for training", type=int, default=100)
